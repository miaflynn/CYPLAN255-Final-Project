...
def group_points_by_poly_naics_year(
    ...
    """
    Groups all the business location points by polygon ID, naics_code and year.
    ...
    tract_naics_year = (
        points
        .groupby([id_col, 'naics_code', year_col, 'status'])
        .size()
        .reset_index(name='count')
        .pivot(index=[id_col, 'naics_code', year_col], columns='status', values='count')
        .fillna(0)
        .reset_index()
        .sort_values(year_col)
    )
